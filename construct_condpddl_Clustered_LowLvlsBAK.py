...
try:
    ...
    task = pddl_pars.open(domain_filename=domainfile, task_filename=problemfile) 
    ...
    for trans_id in range(nb_total_actions):
        ...
        if trans_id in task.indices_actions_no_effects:
            ...
            # print(low_lvl_name_clean)µ

            dico_map_newId_PddlId[trans_id] = act.name

            lla_id = int(low_lvl_name_clean[1:])


            highlvlid = dico_lowlvl_highlvl[lla_id]
    ...
    # ça fonctionnait, à chque hla_id donnait un llow_id basé sur un comptage (et donc ignorait le format a0-*-+ etc)

    # ET ce comptage, appartenant au dico: dico_highlvlid_lowlvlactions

    #       EST ensuite utilisé dans la construction des clusters

    #              DONC, ce comptage ça n'est PAS les IDS du PDDL,
    #
    #
    #                       Pour cela (si nécessaire) il faut faire une fonction qui MAP comptage fais dans
    #
    #                           construct_condpddl_Clustered_LowLvls.py et clusters_llas.py AVEC les vrais noms des 
    #
    #                                                   llas qui sont dans le domain.pddl !!!!
    #
    #
