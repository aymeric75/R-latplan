...
#parser.add_argument("--filter_out_dt1_by_coverage", choices=["true", "false"], required=True)
...
#filter_out_dt1_by_coverage = True if args.filter_out_dt1_by_coverage == "true" else False  # remove the branches which coverage is < to a certain threshold
...
# input like (z0) or (not (z0))
# output like z_0 or not(z_0)
def fromunderscore_to_normal(input):
    if "not" in input:
        integer = str(int(''.join(x for x in str(input) if x.isdigit())))
        return "not(z_"+integer+")"
    else:
        integer = str(int(''.join(x for x in str(input) if x.isdigit())))
        return "z_"+integer
...
# input like (z0) or (not (z0))
# output like z_0_1 or z_0_0
def fromunderscore_to_completeunderscore(input):
    if "not" in input:
        integer = str(int(''.join(x for x in str(input) if x.isdigit())))
        return "z_"+integer+"_0"
    else:
        integer = str(int(''.join(x for x in str(input) if x.isdigit())))
        return "z_"+integer+"_1"


#input like (z0)
# output like z_0_0
def fromnormaltounderscores(input):
...
# print(fromunderscore_to_normal("(z11)"))
# exit()
...
# atom is like (z0)
# dataset is every line a transition
# and columns are like z_0 etc not(z_0)
def coverage_of_atom(atom, dataset):
    column_name = fromunderscore_to_normal(atom)
    indices = dataset.index[dataset[column_name] == 1].tolist()
    return indices



# (not (z1))
def coverage_of_a_list_of_atoms(atoms_list, dataset, format="fromunderscore_to_normal"):
    column_names = []
    # print(len(dataset)) 
    # print(atoms_list)
    # print(dataset.head())
    ...
    for atom in atoms_list:
        ...
        if format == "fromunderscore_to_normal":
            column_names.append(fromunderscore_to_normal(atom))
        elif format == "fromunderscore_to_completeunderscore":
            column_names.append(fromunderscore_to_completeunderscore(atom))
        elif format == "fromnormaltounderscores":
            column_names.append(fromnormaltounderscores(atom))

 
    # retrieve the indices where all atoms agree (are 1)
    indices = dataset.index[dataset[column_names].all(axis=1)].tolist()
    # print(indices)
    # print("la fin")
...
for num_action in range(0, 22):
    ...
    # COMPUTE REAL COVERAGE of each branch (replace #samples)


    for eff, conds in preconds_perEff_perAction[num_action].items():

        for ijijijij, cond in enumerate(conds):
        
            # list of transitions IDs which "preconditions" <=> cond[:-1]
            list_covered = coverage_of_a_list_of_atoms(cond[:-1], X_train, "fromnormaltounderscores")
                            
            indices_where_effect_is_1 = Y_train[Y_train[eff] == 1].index
            indices_where_effect_is_1 = indices_where_effect_is_1.tolist()
            real_coverage = common_elements(list_covered, indices_where_effect_is_1)

            preconds_perEff_perAction[num_action][eff][ijijijij][-1] = len(real_coverage)
            #exit()
    ...
    # d'une part un dt1 qui dit pour un certain effet pour une certaine high lvl action

    #    quels branche / suite / set de preconditions sont activées

    #       si on a ['(z11)', '(z3)', '(z13)'] comme branche, pour add_0, avec 23 samples, ça

    #           veut dire que sur les 192 samples (un peu moins car on ne prend que le X_train ie 0.8 * 192)

    ###                         alors 23 samples (ie 23 transitions) ont à la fois ces 3 atoms activés dans le X

    ###                     POURTANT si j'inspecte le X pour l'activation des ces 3 atoms en même temps

    #                           je n'obtiens que 4

    ####                        1) calcul le coverage uniquement sur le X_train

    ####                        2) essais de retrouver peu à peu les values de l'arbre




    #coverage_of_a_list_of_atoms()
    ...
    for num_action in range(0, 22):
        ...
        if len(beauty_paths) > 0:
            ...
            for ijijijij, l in enumerate(beauty_paths_sorted):
                ...
                list_covered = coverage_of_a_list_of_atoms(l[:-1], new_entire_X_, "fromnormaltounderscores")
                      
                indices_where_action_is_activated = np.where(all_Ys[:, num_action] == 1)[0]
        
                # print(type(list_covered))
                # print(type(indices_where_action_is_activated))
               

                real_coverage = common_elements(list_covered, indices_where_action_is_activated.tolist())
                ...
                # preconds_perEff_perAction[num_action][eff][ijijijij][-1] = len(real_coverage)
                # #exit()
                beauty_paths_sorted[ijijijij][-1] = len(real_coverage)
