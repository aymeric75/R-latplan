...
# print(dico_highlvlid_lowlvlactions.keys())
# exit()
###################
...
for num_action in range(0, nber_hlas):
    ...
    for penal in (True, False):
        ...
        for kkk, vvv in dico__.items():
            ...
            # MAP back the lla_id to the ids of the cluster

            real_indices = list(set(list(dico_highlvlid_lowlvlactions[num_action].keys())))

            dico_fakeIn_RealInd = {}
     
            if kkk == "both":
                for indexx in range(len(vvv[0])):
                    dico_fakeIn_RealInd[indexx] = real_indices[indexx]   
            else:
                for indexx in range(len(vvv)):
                    dico_fakeIn_RealInd[indexx] = real_indices[indexx]

            # print(dico_fakeIn_RealInd)
            # print("dico_fakeIn_RealInd")
            # print(dico_fakeIn_RealInd)
            # print("clusters_")
            # print(clusters_)
            clusters_real = {
                cluster_id: [dico_fakeIn_RealInd[fake_id] for fake_id in fake_ids]
                for cluster_id, fake_ids in clusters_.items()
            }

            with open(base_dir+"/clusterings/"+str(num_action)+"_clusters_"+str(penal)+"_"+str(kkk)+".txt", 'w') as fff:
                for value in clusters_real.values():
